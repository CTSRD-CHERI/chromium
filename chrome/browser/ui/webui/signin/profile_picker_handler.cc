...
base::Value::Dict CreateProfileState(const ProfileAttributesEntry* entry,
...
void ProfilePickerHandler::HandleGetProfileState(
    const base::Value::List& args) {
  AllowJavascript();
  CHECK_EQ(2U, args.size());
  const base::Value& callback_id = args[0];
  ...
  base::Value::Dict dict = CreateProfileState(entry, avatar_icon_size);
  ResolveJavascriptCallback(callback_id, dict);
}
...
base::Value::List ProfilePickerHandler::GetProfilesList() {
  ...
  const int avatar_icon_size =
      kProfileCardAvatarSize * web_ui()->GetDeviceScaleFactor();
  for (const ProfileAttributesEntry* entry : entries) {
    profiles_list.Append(CreateProfileState(entry, avatar_icon_size));
  }
