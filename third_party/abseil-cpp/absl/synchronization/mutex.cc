...
static bool AtomicSetBits(std::atomic<uintptr_t>* pv, ptraddr_t bits,
                          ptraddr_t wait_until_clear) {
  for (;;) {
    uintptr_t v = pv->load(std::memory_order_relaxed);
    if ((v & bits) == bits) {
      return false;
    }
...
static const ptraddr_t kMuDesig = 0x0002L;
...
// reader will wake it.
static const ptraddr_t kMuWrWait = 0x0020L;
