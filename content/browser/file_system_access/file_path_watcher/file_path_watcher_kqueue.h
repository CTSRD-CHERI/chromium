...
class FilePathWatcherKQueue : public FilePathWatcher::PlatformDelegate {
 ...
 private:
  class EventData {
   public:
    EventData(const base::FilePath& path,
              const base::FilePath::StringType& subdir)
        : path_(path), subdir_(subdir) {}
    base::FilePath path_;                // Full path to this item.
    base::FilePath::StringType subdir_;  // Path to any sub item.
  };
  ...
  static size_t EventsForPath(base::FilePath path, EventVector* events);

  // Fills |events| with one kevent for |path|. Returns 1 if a file descriptor
  // for |path| could be created, or 0 otherwise.
  static size_t EventForItem(const base::FilePath& path, EventVector* events);
  ...
  static uintptr_t FileDescriptorForPath(const base::FilePath& path);
  ...
  std::unique_ptr<base::FileDescriptorWatcher::Controller>
      kqueue_watch_controller_;
};

}  // namespace content

#endif  // CONTENT_BROWSER_FILE_SYSTEM_ACCESS_FILE_PATH_WATCHER_FILE_PATH_WATCHER_KQUEUE_H_
